...
exports.QuizElements = function (globalVariables, classContainer) {
        ...
        render() {
            ...
            var _displayQuizHeadInfo = (x, y, w, h) => {
                ...
                var quizLabelDisplay = () => {
                    ...
                    quizLabel.content = autoAdjustText(text, w, h / 2, 15, "Arial", this.quizInfoManipulator).text;
            ...
            var _displayPreviewButton = (x, y, w, h) => {
                ...
                this.previewButtonManipulator.addEvent('click', _previewHandler);
            };
            var _displayQuizSaveButton = (x, y, w, h) => {
                ...
                this.saveQuizButtonManipulator.addEvent('click', this.saveQuiz);
            }
    ...
    class QuestionVue extends Vue {
        ...
        clickHandler(){
            drawings.component.clean();
            this.parentQuiz.parentFormation.quizManager.questionCreator.explanation = null;
            this.selected = true;
            let quizManager = this.parentQuiz.parentFormation.quizManager,
                quiz = quizManager.quiz,
                tabQuestions = quiz.tabQuestions,
                questionCreator = quizManager.questionCreator;

            if (quiz.tabQuestions[quizManager.indexOfEditedQuestion]) {
                quiz.tabQuestions[quizManager.indexOfEditedQuestion].redCrossManipulator
                && quiz.tabQuestions[quizManager.indexOfEditedQuestion].redCrossManipulator.flush();
                quiz.tabQuestions[quizManager.indexOfEditedQuestion].tabAnswer.forEach(answer => {
                    if (answer.popIn) {
                        quizManager.questionCreator.manipulator.remove(answer.popIn.manipulator);
                    }
                })
            }
            if(quizManager.indexOfEditedQuestion !== tabQuestions.indexOf(this)){
                tabQuestions[quizManager.indexOfEditedQuestion].selected = false
            }

            quizManager.indexOfEditedQuestion = tabQuestions.indexOf(this);
            quizManager.displayQuestionsPuzzle(null, null, null, null, quizManager.questionPuzzle.indexOfFirstVisibleElement);
            questionCreator.loadQuestion(this);
            questionCreator.display(questionCreator.previousX, questionCreator.previousY, questionCreator.previousW, questionCreator.previousH);
            questionCreator.explanation && questionCreator.manipulator.remove(questionCreator.explanation.manipulator);
        }
    }
    ...
    class QuestionVueAdmin extends QuestionVue {
        constructor(question, quiz) {
            super(question, quiz);
            this.manipulator.addEvent('click',() => this.clickHandler.call(this));
        }
