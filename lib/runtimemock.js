    ...
    return {
        ...
        json: function (component) {
            return JSON.stringify(component, function (key, value) {
                return key === "parent" || key === "sizes" ? undefined : value;
            });
        },
        random: function () {
...
var runtimeRegister = function (register) {
    ...
    function addBBox(id, x, y, width, height) {
        lastFact.bboxes.push({id: id, width: width, height: height, x: x, y: y});
    }
    ...
    return {
            ...
            if (tag === "textarea") {
                elem.target.addEventListener("input", function (event) {
                    ...
                    addHistory({
                        type: 'event',
                        name: "input",
                        event: {text: elem.target.value},
                        component: component.mock.id
                    });
                });
            }
        ...
        create: function (tag) {
            ...
            elem.target.getBoundingClientRect = function () {
                ...
                addBBox(elem.mock.id, bbox.x, bbox.y, bbox.width, bbox.height);
                return bbox;
            }
            return elem;
        },
