...
exports.mockRuntime = function() {
    ...
    return {
        ...
        createDOM(tag) {
            ...
            if (tag === 'textarea' || 'input') {
                elem.enter = text=> {
                    elem.value = text;
                    elem.listeners && elem.listeners.input && elem.listeners.input({});
                };
                elem.gettingOut = text=> {
                    elem.value = text;
                    elem.listeners && elem.listeners.blur && elem.listeners.blur({});
                };
...
exports.registerRuntime =  function(targetRuntime, register) {
    ...
    return {
        createDOM(tag) {
            ...
            if (tag==="textarea" || tag==="input"){
                elem.target.addEventListener("input", function(event){
                    addHistory({type:'event', name:"input", event:{text:elem.target.value}, component:elem.mock.id});
                    elem.mock.enter(elem.target.value);
                });
                elem.target.addEventListener("blur", function(event){
                    addHistory({type:'event', name:"blur", event:{text:elem.target.value}, component:elem.mock.id});
                    elem.mock.gettingOut(elem.target.value);
                });
            }
            return elem;
        },
        ...
        focus(component) {
            target.focus(component.target);
            mock.focus(component.mock);
        },
        first(component) {
            return component.children.length===0 ? null : component.children[0];
        },
        ...
        addEvent(component, eventName, handler) {
            handler.wrapper = (event)=> {
                ...
                if (!event.proc && (eventName!=="blur" && eventName!=="input")){
                    addHistory({type:'event', name:eventName, event:hEvent, component:component.mock.id});
                    event.proc = true;
                }
                handler(event);
            };
            (eventName!=="blur" && eventName!=="input") && mock.addEvent(component.mock, eventName, handler);
