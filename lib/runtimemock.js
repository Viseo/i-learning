            ...
            elem.getBoundingClientRect = ()=> {
                ...
                if (tag === 'rect') {
                        ...
                        return {
                            ...
                            width: elem["font-size"]*2,
                            height: elem.text.length*elem["font-size"]
                        };
                }
                else if (tag === 'path') {
                    let maxx = null;
...
exports.registerRuntime =  function(targetRuntime, register) {
    ...
    return {
        ...
        setCookie(cookie){
            return target.setCookie(cookie);
        },
        removeSelection(){
            return target.removeSelection();
        },
        speechSynthesisSpeak: function(textToSay){
            return target.speechSynthesisSpeak(textToSay);
        },
        speechSynthesisCancel: function(){
            return target.speechSynthesisCancel();

        },
        fullScreen: function(){
            return target.fullScresen();
        },
        selectionStart: function(component){
            return target.selectionStart();
        },
        setSelectionRange: function(component, caretPos){
            return target.setSelectionRange(caretPos, caretPos);
        },
        createTextRange: function(component){
            return target.createTextRange();
        },
        select: function(component){
            return target.select();
        },
        twinBcrypt: function(label){
            return target.twinBcrypt(label);
        },
        now() {
            return target.now();
        },
