...
function loadPageWithFormation(pageName, mockResponses = {}, formationJson){
    let page = main(mockResponses);
    page.state["loadPresenter" + pageName](page.state.createFormation(formationJson));
    return page;
}
...
function clickAlone(root, element) {
    let elt = retrieve(root, "[" + element + "]");
    if (elt.handler.parentManip) {
        elt.handler.parentManip.listeners['click']();
    } else {
        elt.listeners['click']();
    }
}
...
exports.assertMessage = assertMessage;
exports.assertMissing = assertMissing;
exports.loadPageWithFormation = loadPageWithFormation;
