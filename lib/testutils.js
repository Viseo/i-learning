...
function retrieveDB(path, listener, callback) {
    ...
    rs.on('end', () => {
        listener.data = data.slice(0, -1).split("\n");
        callback();
    });
}
...
function checkScenario(play, scenario, root, runtime, done) {
    ...
    rl.on('line', function (line) {
        ...
        if (fact.type === 'init') {
            ...
            if (fact.type === 'event') {
                if (fact.name==="input" && fact.event.text){
                    runtime.anchor(root).getElement(fact.component).enter(fact.event.text);
                }
                else if (fact.name==="blur" && fact.event.text){
                    runtime.anchor(root).getElement(fact.component).gettingOut(fact.event.text);
                }
                else {
