...
function customClick(root, buttonName, data){
    let elt = retrieve(root, "[" + buttonName + "]");
    assert(elt);
    elt.listeners['click'](data);
}
...
function rightClick(root, eltName, data){
    let clickElement = retrieve(root, "[" + eltName + "]");
    assert(clickElement);
    clickElement.listeners['contextmenu'](data);
}
...
function mouseEnterElement(root, elementName) {
    let element = retrieve(root, "[" + elementName + "]");
...
function mouseDown(root, elementName, event) {
    ...
    if (element.listeners['mousedown']){
        element.listeners['mousedown'](eventDefault);
    }else{
        element.handler.parentManip.listeners['mousedown'](eventDefault);
    }
}
function mouseDownElement(root, elementName, event) {
    let element = retrieve(root, "[" + elementName + "]"),
        eventDefault = {pageX: element.handler.parent.x,pageY: element.handler.parent.y,
            preventDefault: () => {}};
    if (event) {
        eventDefault = event;
    }
...
function mouseUp(root, elementName, event) {
    ...
    if (element.listeners['mouseup']){
        element.listeners['mouseup'](eventDefault);
    }else{
        element.handler.parentManip.listeners['mouseup'](eventDefault);
    }
}
function mouseUpElement(root, elementName, event) {
    let element = retrieve(root, "[" + elementName + "]"),
        eventDefault = {pageX: element.handler.parent.x,pageY: element.handler.parent.y,
            preventDefault: () => {}};
    if (event) {
...
function mouseUpElementOnAnother(root, draggedName, droppedName) {
    let dragElement = retrieve(root, "[" + draggedName + "]"),
        dropElement = retrieve(root, "[" + droppedName + "]");
    dragElement.listeners['mouseup']({pageX: dropElement.handler.x, pageY: dropElement.handler.y,
        preventDefault: () => {}});
}
function mouseUpOnAnother(root, draggedName, droppedName) {
    let dragElement = retrieve(root, "[" + draggedName + "]"),
        dropElement = retrieve(root, "[" + droppedName + "]");
    if (dragElement.listeners['mouseup']){
        dragElement.listeners['mouseup']({pageX: dropElement.handler.x, pageY: dropElement.handler.y,
            preventDefault: () => {}});
    }else{
        dragElement.handler.parentManip.listeners['mouseup']({pageX: dropElement.handler.x, pageY: dropElement.handler.y,
            preventDefault: () => {}});
    }
}
...
function mouseMove(root, elementName, event) {
    ...
    if (element.listeners['mousemove']){
        element.listeners['mousemove'](eventDefault);
    }else{
        element.handler.parentManip.listeners['mousemove'](eventDefault);
    }
}
function mouseMoveElement(root, elementName, event) {
    let element = retrieve(root, "[" + elementName + "]"),
        eventDefault = {pageX: element.handler.parent.x,pageY: element.handler.parent.y,
            preventDefault: () => {}};
    if (event) {
...
function dblclickElement(root, elementName) {
    let elt = retrieve(root, "[" + elementName + "]");
    assert(elt);
    elt.listeners['dblclick']();
}
...
exports.dblclickElement = dblclickElement;
...
exports.mouseUpElementOnAnother = mouseUpElementOnAnother;
exports.onChangeElement = onChangeElement;
exports.when = when;
exports.retrieve = retrieve;
exports.mouseDownElement = mouseDownElement;
exports.mouseMoveElement = mouseMoveElement;
exports.mouseUpElement = mouseUpElement;
exports.rightClick = rightClick;
exports.customClick = customClick;
exports.mouseUpOnAnother = mouseUpOnAnother;