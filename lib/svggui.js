...
exports.Gui = function(svg, param) {
    ...
    class ScrollablePanel {
        constructor(width, height) {
            ...
            this.back = new svg.Rect(width, height).color([], 0, []);
            this.content = new svg.Translation();
        ...
        processKeys(keycode) {
            if (isLeftArrow(keycode)) {
                this.moveContent(this.content.x+100, this.content.y, 'contentH');
            }
            else if (isUpArrow(keycode)) {
                this.moveContent(this.content.x, this.content.y+100, 'contentV');
            }
            else if (isRightArrow(keycode)) {
                this.moveContent(this.content.x-100, this.content.y, 'contentH');
            }
