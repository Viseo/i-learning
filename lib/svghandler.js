...
exports.SVG = function(runtime) {
    ...
    class TextItem extends DomElement {
        ...
        setCaretPosition(caretPos) {
            if(svgr.createTextRange(this.component)) {
                let range = svgr.createTextRange(this.component)();
                range.move('character', caretPos);
                range.select();
            }
            else if(svgr.selectionStart(this.component) || svgr.selectionStart(this.component) === 0) {
                svgr.focus(this.component);
                svgr.setSelectionRange(this.component, caretPos);
            }
            else {
                svgr.focus(this.component);
            }
    ...
    class Video extends DomElement {
    ...
    class SVGString extends Shape {
        constructor (string) {
            super();
            this.component = svgr.create("path");
            this.drawing = string;
            svgr.attr(this.component, "data", this.data);
            svgr.attr(this.component, "type", "image/svg+xml");
            this._draw();
        }
    ...
    function activeElement(){
        return svgr.activeElement();
    }
    function setCookie(cookie) {
        return svgr.setCookie(cookie);
    }
    function removeSelection(){
        return svgr.removeSelection();
    }
    function speechSynthesisCancel() {
        return svgr.speechSynthesisCancel();
    }
    function speechSynthesisSpeak(textToSay) {
        return svgr.speechSynthesisSpeak(textToSay);
    }
    function fullScreen(){
        return svgr.fullScreen();
    }
    function twinBcrypt(label){
        return svgr.twinBcrypt(label);
    }
    return {
        Screen : Screen,
        Block : Block,
        ...
        speechSynthesisCancel: speechSynthesisCancel,
        speechSynthesisSpeak: speechSynthesisSpeak,
        fullScreen: fullScreen,
        twinBcrypt: twinBcrypt,
        random : random,
        timeout : timeout,
        interval : interval,
